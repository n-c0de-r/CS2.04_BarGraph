import java.io.*;
/**
 * This class takes an input text file, iterates over every single character
 * and counts their occurrence. Only ASCII characters from A-Z are considered.
...
public class CountChars {
	...
	/**
	 * prints out a histogram where stars represent a
	...
	/**
	 * Finds the highest number of a letter, 
	 * using this as the scale maximum. Recursive function.
	...
	public static int findMax(int[] values, int start, int value)
	{
	   ...
	   else return findMax(values, start + 1, Math.max(value, values[start]));
	}
}
